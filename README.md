...
Main purpose of this project is to test fault tolerance scenarios and confirm those are working as expected with results/logs.

### How to setup and configure this project
1. Fault Tolerance policies used in this application are:
	+ bulkheadisolation - Keeping our application stable when under heavy load.
	...
	+ Wait and Retry - Our application targets to hit test / mock API. If calling an API throws some error, the retry policy should wait for 5 seconds and call api
	again. Likewise, it should retry 3 times with interval of 1,5,10 seconds.
	+ Circuit Breaker - 
		* Open circuit - Flow/Circuit is breaked in between for some time
		* Closed circuit - Flow/Circuit is running and can send the requests
...
properties :
+ circuitBreakerOnBreakDuration : circuit breaker policy break duration in seconds
+ maxParallelization : Number of actions/requests executing concurrently / parallely
+ maxQueuingActions :Maximum number of actions/requests queuing , waiting for execution slot

Section Name : RemoteServer
properties :
+ baseAddress : Target API to hit. Once Mock API is in place, this property need to be set with Mock API.
